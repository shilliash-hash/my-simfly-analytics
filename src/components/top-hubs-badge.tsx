...
export function TopHubsBadge({ airports }: { airports: AirportExt[] }) {
  ...
  // Bezpieczne zamykanie okna po kliknięciu poza nim lub kliknięciu ESC
  useEffect(() => {
    if (!open) return;
  ...
  // Sortowanie lotnisk od najbardziej dochodowych
  ...
  return (
    <div ref={wrapRef} className="relative">
      ...
      {open && (
        /* 
          - Zero pełnoekranowego bluru (strona główna pod spodem zostaje idealnie ostra)
          - bg-background/60 + backdrop-blur-xl daje głębokie, czyste rozmycie wyłącznie pod samą kartą
          - border-border/40 tworzy tę unikalną, ultra-cienką obwódkę
        */
        <div
          className="panel absolute right-0 z-30 mt-2 w-[min(92vw,28rem)] max-h-[min(80vh,42rem)] overflow-hidden rounded-2xl p-4 shadow-2xl bg-background/60 backdrop-blur-xl border border-border/40 shadow-black/60 animate-in fade-in slide-in-from-top-1 duration-150"
          role="dialog"
        >
          {/* NAGŁÓWEK KARTY */}
              ...
              {sorted.map((a) => (
                <li key={a.icao}>
                  ...
                  <Link
                    ...
                    params={{ id: a.icao }}
                    onClick={() => setOpen(false)}
                    className="group block rounded-lg px-2.5 py-2.5 transition-all duration-150 hover:bg-white/[0.04]"
                  >
                    ...
                    {/* SIATKA STATYSTYK FINANSOWYCH */}
                    <div className="mt-3 grid grid-cols-3 gap-3 border-t border-border/20 pt-2.5 text-[11px] px-1">
                      <MiniStat
                        label="Lifetime PAX"
